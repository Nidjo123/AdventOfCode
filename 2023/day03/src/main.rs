...
fn parse_data(input: &str) -> (Vec<PartNumber>, Vec<Symbol>) {
    ...
    let symbols: Vec<Symbol> = input.lines()
        .enumerate()
        .map(|(y, line)|
...
fn part1(part_numbers: &Vec<PartNumber>, symbols: &Vec<Symbol>) {
    let part_num_sum: usize = part_numbers.iter().filter_map(|part_number| {
        if symbols.iter().any(|symbol| part_number.is_adjacent_to(&symbol.loc)) {
...
fn part2(part_numbers: &Vec<PartNumber>, symbols: &Vec<Symbol>) {
    let gear_ratio_sums: usize = symbols.iter()
        .filter(|symbol| symbol.symbol == '*')
        .map(|symbol| {
            let adjacent_part_numbers: Vec<usize> = part_numbers.iter()
