...
fn parse_data(input: &str) -> Data {
    ...
    let last_map = input.lines().filter(|s| !s.trim().is_empty()).fold(Map::new(), |mut map: Map, line| {
        if let Some(seed_capture) = seeds_regex.captures(line) {
            let seeds: Vec<u64> = parse_ints(seed_capture.name("seed_numbers").unwrap().as_str());
            ...
            return Map { source_name, destination_name, ranges: Vec::new() };
...
fn part2(data: &Data) {
    ...
    for (seed_start, length) in data.seeds.iter().step_by(2).zip(data.seeds.iter().skip(1).step_by(2)) {
